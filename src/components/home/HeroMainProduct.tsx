...
export function HeroMainProduct({ product }: Props) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col border-t-2 border-gray-300 md:border-t-0">
      <Link
        href={`/products/${product.slug}`}
        className="flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-white p-3 sm:p-4 md:p-6"
      >
        <div className="flex h-full max-h-48 items-center justify-center md:max-h-72">
          {product.imageUrl ? (
            <img
      ...
      <div className="border-t-2 border-gray-300 bg-white px-4 py-2.5 md:px-5 md:py-3">
        <Link
          href={`/products/${product.slug}`}
